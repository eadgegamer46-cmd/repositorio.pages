...
function showScreen(id){
  ...
  if(!target){ console.error('Tela não encontrada:', id); return; }
...
function renderSales(){
  ...
  if(!ul) return;
  if(state.sales.length === 0){ ul.innerHTML = '<li>Nenhuma venda registrada.</li>'; return; }
  ul.innerHTML = state.sales.map(s => `<li>${escapeHTML(s.product)} - ${s.qty} un. (${s.date})</li>`).join('');
}
function renderStock(){
  const ul = document.getElementById('stockList');
  if(!ul) return;
  if(state.stock.length === 0){ ul.innerHTML = '<li>Nenhum item em estoque.</li>'; return; }
  ul.innerHTML = state.stock.map(s => `<li>${escapeHTML(s.product)} - ${s.qty} un.</li>`).join('');
}
...
function addStock(){
  ...
  const item = state.stock.find(x => x.product.toLowerCase() === product.toLowerCase());
  if(item){ item.qty += qty; } else { state.stock.push({ product, qty }); }
